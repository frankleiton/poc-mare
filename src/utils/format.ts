...
export const converteUnidade = (
  valor: number,
  to: Unidade,
): number => {
  
  let valorMs: number = valor;

  // Converte de m/s para a unidade de destino
  switch (to) {
    case "MS":
      return valorMs;
    case "KMH":
      return parseFloat((valorMs * 3.6).toFixed(2));
    case "KTS":
      return parseFloat((valorMs / 0.514444).toFixed(2));
    default:
      throw new Error(`Unidade desconhecida: ${to}`);
  }
};

export const mapaUnidade = (unidade: Unidade): string => {
  switch (unidade) {
    case "MS":
      return "m/s";
    case "KMH":
