...
export function usePrevisaoClima(
  ...
  return useQuery({
    ...
    queryFn: ({ signal }) =>
      obterPrevisaoClima(cityCode as number, dias, signal),
 ...
 */
export function usePrevisaoOndas(
  cityCode: number | null | undefined,
  dias = 6,
) {
  return useQuery({
    ...
    queryFn: ({ signal }) =>
      obterPrevisaoOndas(cityCode as number, dias, signal),
    enabled: cityCode != null,
    staleTime: UMA_HORA,
    retry: 1,
  });
